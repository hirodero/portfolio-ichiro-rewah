...
function SplashCursor({
  ...
  className = '',
  followRef = /** @type {import('react').RefObject<{ x: number, y: number, id: number } | null> | null} */ (null),
  followPointer = true,
  FOLLOW_STRENGTH = 1,
  FOLLOW_FLOW = 1
}) {
  ...
  useEffect(() => {
    ...
    function syncAnimation() {
      ...
      if (isVisible && !document.hidden) updateFrame();
    }
    ...
    function splatPointer(pointer) {
      const flow = pointer.id === -2 ? FOLLOW_FLOW : 1;
      let dx = pointer.deltaX * config.SPLAT_FORCE * flow;
      let dy = pointer.deltaY * config.SPLAT_FORCE * flow;
    ...
    function splat(x, y, dx, dy, color) {
      if (COLOR_END) {
        ...
        const right = hexToRGB(COLOR_END);
        const mix = Math.max(0, Math.min(1, x));
        color = {
    ...
    // Observe the chosen content area, using the canvas bounds for coordinates.
    ...
    function handlePointerMove(event) {
      const position = localPosition(event);
      if (!position) { handlePointerLeave(); return; }
      if (!pointerInside) {
        updatePointerDownData(pointers[0], event.pointerId, position.x, position.y);
        pointerInside = true;
        return;
    ...
    function applyFollowTarget(dt) {
      ...
      const color = generateColor();
      const strength = FOLLOW_STRENGTH * dt * 60;
      color.r *= strength;
      color.g *= strength;
      color.b *= strength;
    ...
    if (followPointer) {
      ...
      host.addEventListener('pointerup', handlePointerLeave);
    ...
    return () => {
      ...
      // Release GPU resources on real unmount, without breaking Strict Mode replay.
      queueMicrotask(() => {
  ...
  return (
    <div className={`hero-splash-cursor ${className}`} aria-hidden="true">
