...
const ParticleText = ({
  ...
  idleDrift = 0.7,
  trigger = "mount",
  fontSize = /** @type {string | number} */ ("clamp(3rem, 12vw, 8rem)"),
  fontWeight = /** @type {string | number} */ (800),
  fontFamily = "inherit",
  glow = true,
  className = "",
  style = {},
  onGatherComplete = /** @type {(() => void) | undefined} */ (undefined),
  maxParticles,
  gatherFrom = "scatter",
  glyphAlign = "center"
}) => {
  ...
  useEffect(() => {
    ...
    const startGather = (fromScatter = true) => {
      ...
      particles.forEach(particle => {
        if (fromScatter) {
          if (gatherFrom === "center") {
            particle.x = originX + (particle.seed - 0.5) * 6;
            particle.y = originY + (particle.depth - 0.5) * 6;
          } else {
    ...
    const render = now => {
      ...
      particles.forEach(particle => {
        ...
        const follow = reducedMotion ? 1 : gatherFrom === "center" ? 0.34 : 0.22;
        particle.x += (baseX - particle.x) * follow;
        particle.y += (baseY - particle.y) * follow;

        ctx.globalAlpha = clamp(0.35 + progress * 0.65, 0, 1);
        const grown = gatherFrom === "center" ? 0.2 + progress * 0.8 : 1;
        drawParticle({ ...particle, size: particle.size * grown });
      });
    ...
    const sampleText = async () => {
      ...
      const padding = Math.max(2, Math.ceil(resolvedSize * 0.06));
      ...
      for (let y = 0; y < offscreen.height; y += step) {
        for (let x = 0; x < offscreen.width; x += step) {
          ...
          if (alpha > 40) {
            ...
            const originY = glyphAlign === "top" ? 0 : height / 2 - offscreen.height / 2;
            targets.push({
              x: originX + x,
              y: originY + y,
              alpha: alpha / 255
      ...
      const particleCap = typeof maxParticles === "number"
        ? maxParticles
        : Math.max(900, Math.min(5200, Math.floor((width * height) / 90)));
      const stride = Math.max(1, Math.ceil(targets.length / particleCap));
  ...
  return (
    <span ref={containerRef} className={`particle-text ${className}`} style={style} aria-hidden="true">
